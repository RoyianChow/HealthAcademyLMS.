...
export function QuizResultDialog({ quizId }: QuizResultDialogProps) {
    ...
    async function loadQuizResult() {
      ...
      try {
        ...
        if (!response.ok || !data.quiz) {
          throw new Error(data.error ?? "Unable to load quiz result.");
        }
  ...
  if (!latestAttempt && open && !loading && error) {
    return (
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto">
        {loading || !quiz || !latestAttempt ? (
          <p className="text-sm text-muted-foreground">Loading quiz result...</p>
        ) : (
          <>
